...
#postcodify, .adrsChangeBtn, .adrsChangeCancelBtn, .changeBtn {
	border: 1px solid lightgray;
	color:#515151;
 	outline-color: #d5e4f1;
...
$(document).ready(function() {
	...
	$(".updateBtn").on("click",function(){
		...
		$.ajax({ //겟방식? 포스트방식?
			...
			url : "pdUpdateAjax", //주소 호출 (값이랑 보내는)
			...
			success : function(res) { //성공적으로 넘어갔을때 이 함수 실행하며, 돌아오는 값을 res란 이름으로 받는다.
				console.log(res.result);
					...
					if(res.result == "success"){
						...
						$("#actionForm2").attr("action","prsnlDcmntRead");
						$("#actionForm2").submit();
		...
		$(".adrsChangeCancelBtn").on("click",function(){
		 ...
		 $(".adrsChangeBtn").on("click",function(){
			...
			if("${data.ADRS}" == $("#adrs").val() && "${data.DTL_ADRS}" == $("#dtlAdrs").val() && "${data.PSTCD}" == $("#pstcd").val()){
				...
				console.log("selectEmply: " + $("#selectEmply").val() + " ");
...
<div class="title_wrap">
	<div class="title_table">
		<div class="title_txt">인사서류관리 수정</div>
	</div>
</div>
...
<input type="hidden" name="checkDocNo" id="checkDocNo" value="${data.D_NO}">
<input type="hidden" name="checkEmplyNo" id="checkEmplyNo" value="${data.EMPLY_NO}">
<input type="hidden" name="selectEmply" id="selectEmply" value="${data.EMPLY_NO}">


	<div class="tbl1">
			<div class="left">
				<c:if test="${sAthrtyNo eq 1 or sAthrtyNo eq 2 or sAthrtyNo eq 4 or sAthrtyNo eq 5}">
 					<input type="button" value="수정완료" class="updateBtn">
				</c:if>
				...
				<table class="board1">
					...
					<tbody>
						...
						<tr>
							<td rowspan="3">
								${data.PRSNL_DCMNT_KIND}
							</td>
							<td>문서번호</td>
							<td name="docNo09" id="docNo09">${data.PRSNL_DCMNT_NO}</td>
						</tr>
						<tr>
							<td>작성자</td>
							<td name="writer" id="writer">${data.WRTR_NAME}</td>
						</tr>
						<tr>
							<td>작성일자</td>
							<td name="sysdt" id="sysdt">${data.WRT_DATE}</td>
						</tr>
					</tbody>
				</table><br/>
				<br/>
				...
				<table class="board3">
					...
					<tbody>
						...
						<tr>
							<td>제출용도</td>
							<td colspan="3"><input class="text" type="text" name="dcmntUse" id="dcmntUse" value="${data.DCMNT_USE}"></td>
						</tr>
					</tbody>
				</table><br/>
				<br/>
