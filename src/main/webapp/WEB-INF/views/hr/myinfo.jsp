...
<head>
<meta charset="UTF-8">
<title>Vanilla ERP My Info</title>
<!-- 헤더 -->
...
.newBtn, .cancelBtn, .addnumBtn, #changepass {
...
$(document).ready(function() {
	...
	$("#solun").val("${data.SOLAR_LUNAR}");
	$("#gender").val("${data.GNDER}");
	$(".select").val("${data.EDCTN}");
	...
	$("#changepass").on("click",function(){
		...
		content = "<form action=\"#\" id=\"actionForm2\" method=\"post\"><input type=\"hidden\" name=\"sEmplyNo\" id=\"sEmplyNo\" value=\"${sEmplyNo}\">비밀번호* <br/>"
		content += "<input type=\"password\" class=\"text\" name=\"psrd\" id=\"psrd\">"
		content += "<br/><br/>비밀번호 확인* <br/>"
		content += "<input type=\"password\" class=\"text\" name=\"psrd2\" id=\"psrd2\"></form>"
		
		makeTwoBtnPopup(3, "비밀번호 수정", content, true, 400, 500, null, "수정", function() {
			
			if($("#psrd").val() != $("#psrd2").val()) {
				 makeAlert(4, "알림", "비밀번호와 확인이 일치하지 않습니다.", null);
			} else {
				//비밀번호 아작스
				...
				$.ajax({
					...
					success : function(result) {
						if(result.res == "success") {
							...
							makeAlert(4, "확인", "비밀번호가 수정되었습니다.", true, function(){
								closePopup(3);
							});
							
							
						} else if(result.res == "fail") {
							makeAlert(4, "실패", "비밀번호 수정에 실패하였습니다.", true, null);
						} else {
							makeAlert(4, "경고", "비밀번호 수정 중 문제가 발생하였습니다.", true, null);
						}
					},
...
<%-- 현재 페이지 해당 메뉴번호 지정 --%>
	<c:param name="menuNo" value=""></c:param> <%-- gnb --%>
</c:import>
<!-- 구현내용 -->
<div class="title_wrap">
	<div class="title_table">
		<div class="title_txt">나의정보 수정</div>
	</div>
</div>
<div class="contents_area">
	<!-- 내용은 여기에 구현하세요. -->
	
<form action="myinfo2" id="backForm" method="post">
	...
	<table class="board2">
		...
		<tbody>
			...
			<tr>
				...
				<td><input type="button" name="changepass" id="changepass" value="비밀번호 수정"/></td>
				<td></td>
			</tr>
