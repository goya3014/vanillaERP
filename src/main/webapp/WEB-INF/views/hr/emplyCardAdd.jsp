...
$(document).ready(function() {
	...
	$(".newBtn").on("click",function(){
		...
		if(cnt2 > 0){
			makeAlert(1, "알림", "필수 입력사항이 입력되지 않았거나 오류가 있습니다.", null);
			...
			$.ajax({ //겟방식? 포스트방식?
				...
				url : "empWriteAjax", //주소 호출 (값이랑 보내는)
				...
				success : function(res) { //성공적으로 넘어갔을때 이 함수 실행하며, 돌아오는 값을 res란 이름으로 받는다.
					if(res.result == "success"){
						console.log($("#picname").val());
						makeAlert(1, "알림", "등록되었습니다.", null);
						$("#page").val("1");
						location.href="emplyCard"
				...
				error : function(request, status, error) { //값이 돌아오지 않는 등 실행 중 에러 발생 시 이 함수 실행
					if("${res}" == 1) {
						makeAlert(1, "알림", "이메일이 중복되었습니다.", null);
					} else if("${res}" == 2) {
	...
	$(".cancelBtn").on("click",function(){
			makeAlert(1, "알림", "정말 취소하시겠습니까?", null);
			$("#backForm").submit();
	});
...
<div class="title_wrap">
	<div class="title_table">
		<div class="title_txt">인사카드관리 등록</div>
	</div>
</div>
<div class="contents_area">
	<!-- 내용은 여기에 구현하세요. -->
	
<form action="emplyCard" id="backForm" method="post">
...
<form action="#" id="actionForm">
	...
	<input type="button" value="등록" class="newBtn">
	...
	<table class="board2">
		...
		<tbody>
			<tr class="gong">
				...
				<div class="pic"></div>사원번호 미정</td> <!-- 사진 넣는 div:pic -->
				<td colspan="4">* 표시는 필수 입력사항입니다.</td>
			</tr>
			<tr>
				<td>이름* </td>
				<td> <input type="text" class="text" name="eName"></td>
				<td> </td>
				<td></td>
			</tr>
			<tr>
				<td>생년월일* </td>
				<td><input type="number" class="text" placeholder="20200625" name="eBirth"></td>
				<td><div class="fix"><input type="radio" name="solun" value="0"/>양력 <input type="radio" name="solun" value="1"/>음력</div></td>
				<td class="gender">성별* <input type="radio" name="gender" value="1"/>여 <input type="radio" name="gender" value="0"/>남 </td>
			</tr>
			<tr>
				<td>주소* </td>
				<td colspan="2"><div class="fix"><input type="text" class="text postcodify_address" id="add" name="adrs" value="">
				<input type="text" class="text postcodify_postcode" id="addnum" name="pstcd" value="">
			...
			<tr>
				<td rowspan="4"></td>
				<td colspan="4"><input type="text" class="text postcodify_details" id="detailAdd" name="dtlAdrs" placeholder="상세주소" value=""></td>
			</tr>
			<tr>
				<td>휴대전화* </td>
				<td><input type="number" class="text" placeholder="01012345678" name="clphnNo"></td>
				<td>예비번호 </td>
				<td><input type="number" class="text2" placeholder="01023456789" name="spareNo"></td>
			</tr>
			<tr>
				<td>이메일* </td>
				<td><input type="text" class="text" name="email"></td>
				<td>비밀번호* </td>
				<td><input type="password" class="text" name="pswrd"></td>
			</tr>
		</tbody>
	</table>
	</form>
